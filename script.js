class App {
    ...
    async loadAllData() {
        try {
            ...
            let allLessons = [];
            vocabResults.filter(d => d && d.lessons).forEach(d => { allLessons = allLessons.concat(d.lessons); });
    ...
    populatePageCheckboxes() {
        ...
        this.selectedLessons.forEach(lessonNum => {
            ...
            if (lesson && lesson.pages) {
                lesson.pages.forEach(p => { availablePages.push({ lessonNum: lesson.number, pageNum: p.number, id: `${lesson.number}-${p.number}` }); });
            }
        });
