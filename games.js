...
class SnakeGameLogic {
    ...
    handleGlobalKeyDown(e) { const gameKeys = ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight']; if (gameKeys.includes(e.key) && (this.status === 'playing' || !this.overlay.classList.contains('hidden'))) { e.preventDefault(); } if (this.gameType === 'snake' && this.status === 'playing') { this.handleKeyInput(e); } else if ((this.status === 'idle' || this.status === 'gameover') && (e.key === 'Enter' || e.key === ' ')) { if (!this.overlay.classList.contains('hidden')) this.startGame(); } }
